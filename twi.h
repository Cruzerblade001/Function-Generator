...
{
	lcd |= 0x01;								//--- Enable Backlight Pin & Select Data Register By RS = 1
	PCF8574_write(lcd);							//--- Send Data From PCF8574 to LCD PORT	
	twi_lcd_4bit_send(x);						//--- Function to Write 4-bit data to LCD
}
...
{	
	...
	//twi_lcd_cmd(0x03);				//--- Initialize Sequence
	//twi_lcd_cmd(0x0E);				//--- Initialize Sequence
	twi_lcd_cmd(0x02);				//--- Return to Home
	twi_lcd_cmd(0x28);				//--- 4-Bit Mode 2 - Row Select
	twi_lcd_cmd(0x0E);				//--- 
	twi_lcd_cmd(0x01);				//--- Clear LCD
	//twi_lcd_cmd(0x80);				//--- Auto increment Cursor
	//twi_lcd_cmd(0x06);				//--- Row 1 Column 1 Address
	twi_lcd_msg("FUNCTION GEN");
	//twi_lcd_cmd(0x06);	//--- String Send to LCD
